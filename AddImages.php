...
function checkIfExists($imagePath)
{
	...
	if ($totalRows_MediaQuery > 0 ) {
		...
		print "ADDING $imagePath to the MEDIA table.\n<br />";
		return true;
	}
}
...
<?php 
	...
	if (isset($_GET['folder'])) {
		...
		iterateThroughImages($_GET['folder']);
	} else {
		print "File Listing: \n<br />";
	...
	*/
	...
	function iterateThroughImages($dirPath) {
		foreach (new DirectoryIterator($dirPath) as $file) {
...
*/
			...
			switch ($fileExtension) { 
				case "jpg" : 
					if (checkIfExists($file->getPathname())) {
