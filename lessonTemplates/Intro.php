<div id="ChallengeWrapper"> <!-- ChallengeWrapper. Constrained content area for presentation elements. -->
    
  <div id="ChallengeTitle">
    <img src="assets/images/intrologo.png" width="175" height="87" alt="Projector Logo" class="ProjectorLogo" />
    <h1 class="ProjectorTitle">Pearson Foundation Learning Labs</h1>
    <h2 id="ChallengeTitleProject" class="ProjectTitle"></h2>
    <h3 id="ChallengeTitleAuthor" class="ProjectAuthor"></h3>
    <img id="ChallengeTitleBtn" src="assets/images/challengeintro_arrow.png" width="60" height="60" style="padding-top:40px" />
  </div>

  <div id="ChallengeGroupA" class="landscapex3">
    <div id="ChallengeImageA1" class="challengeImage1">
      <img id="ChallengeImageA1Img" src="" title="" alt="">
    </div>
    <div id="ChallengeImageA2" class="challengeImage2">
      <img id="ChallengeImageA2Img" src="" title="" alt="">
    </div>
    <div id="ChallengeImageA3" class="challengeImage3">
      <img id="ChallengeImageA3Img" src="" title="" alt="">
    </div>
  </div>
  
  <div id="ChallengeGroupB" class="landscapex3">
    <div id="ChallengeImageB1" class="challengeImage1">
      <img id="ChallengeImageB1Img" src="" title="" alt="">
    </div>
    <div id="ChallengeImageB2" class="challengeImage2">
      <img id="ChallengeImageB2Img" src="" title="" alt="">
    </div>
    <div id="ChallengeImageB3" class="challengeImage3">
      <img id="ChallengeImageB3Img" src="" title="" alt="">
    </div>
  </div>
  
  <div id="ChallengeTextWrapper">
    <div id="ChallengeText">
    </div>
  </div>
  
  <div id="ChallengePaused">
    <img id="ChallengePausedImg" src="assets/images/challengeintro_arrow.png" width="60" height="60" />
  </div>
  
</div> <!-- ChallengeWrapper Ends --> 
<div id="challengeAudioPlayer" class="jp-jplayer"></div>