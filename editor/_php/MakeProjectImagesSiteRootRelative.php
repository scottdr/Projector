...
function makeFieldSiteRelative($fieldName,$rowData,$projectId) {
		global $projectorConnection;
		
		$url = $rowData[$fieldName];	
		if (strlen($url) > 0 ) {
			$firstChar = $url[0];
			if ($firstChar != '/') {
				$url = '/' . $url;
				$sqlCommand = sprintf("UPDATE Projects SET %s=%s WHERE Id=%s",
										 GetSQLValueString($fieldName, "text"),
...
function makeMediaUrlSiteRelative($projectId) {
	...
	if ($result = mysqli_query($projectorConnection, "SELECT * FROM Projects WHERE ProjectId = " . $projectId)) {
			$numMedia = mysqli_num_rows($result);
			printf("Select returned %d rows.\n", $numMedia);
			$result->data_seek(0);
			while ($row = $result->fetch_assoc()) {
				makeFieldSiteRelative("ImgSmall",$row,$projectId);
				makeFieldSiteRelative("ImgMedium",$row,$projectId);
				makeFieldSiteRelative("ImgLarge",$row,$projectId);
